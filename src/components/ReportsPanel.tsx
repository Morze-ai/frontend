      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 sidebar-scroll">
        ...
        {sortedReports.map((report, index) => {
          ...
          return (
            <article
              key={report.model_id}
              className={`rounded-xl border p-3 ${index === 0 ? "border-cyan-500/40 bg-cyan-500/5" : "border-slate-700/60 bg-slate-900/60"}`}
            >
              <div className="flex items-start justify-between gap-3">
                ...
                {pdfUrl && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-1 text-[11px] px-2 py-1 rounded-full border border-cyan-500/40 text-cyan-300 hover:border-cyan-400/60 hover:bg-cyan-500/10"
                  >
