...
const MODEL_CONFIG: Record<string, { name: string; type: string }> = {
  ...
  Logistic: { name: 'Logistic regression classifier', type: 'klasyfikator liniowy' },
  MLP: { name: 'Multi-layer Perceptron classifier', type: 'klasyfikator nieliniowy' },
};

export function ReportsPanel({ data, onClose }: ReportsPanelProps) {
  const sortedReports = [...data.modelReports].sort((a, b) => b.accuracy - a.accuracy);
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 sidebar-scroll">
        <div className="h-32 bg-slate-900/60 rounded-xl p-3 border border-slate-800">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={sortedReports.map((report) => ({ name: report.model_name.replace('_classifier', '').replace('_regression', ''), accuracy: +(report.accuracy * 100).toFixed(1) }))} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" vertical={false} />
              <XAxis dataKey="name" stroke="#475569" fontSize={10} />
              <YAxis stroke="#475569" fontSize={9} domain={[50, 100]} />
              <RTooltip contentStyle={{ backgroundColor: '#0f172a', border: '1px solid #334155', borderRadius: '8px', fontSize: '12px' }}  itemStyle={{ color: "#e2e8f0" }} labelStyle={{ color: "#ffffff" }}formatter={(value: any) => [`${Number(value)}%`, 'Dokładność']}  cursor={{ fill: "rgba(45, 53, 88, 0.34)" }}/>
        ...
        {sortedReports.map((report, index) => {
          const simpleName = report.model_name.replace('_classifier', '').replace('_regression', '');
          const modelConfig = MODEL_CONFIG[simpleName] ?? { name: simpleName, type: report.experiment_name };
          ...
          return (
            <article key={report.model_id} className={`rounded-xl border p-3 ${index === 0 ? 'border-cyan-500/40 bg-cyan-500/5' : 'border-slate-700/60 bg-slate-900/60'}`}>
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-sm font-semibold text-white">{modelConfig.name}</p>
                  <p className="text-[11px] text-slate-400 mt-1">{modelConfig.type}</p>
                </div>
                {pdfUrl && (
              ...
              <div className="grid grid-cols-3 gap-2 text-[11px] mt-3 text-slate-300">
                <div className="rounded-lg bg-slate-950/70 p-2 border border-slate-800">
                  <p className="text-slate-500">Dokładność</p>
